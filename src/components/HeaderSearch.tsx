...
interface HeaderSearchProps {
  placeholder?: string;
  /**
   * Visual scale. The widget is otherwise identical everywhere — `sm` is the
   * compact header/nav bar, `lg` is the prominent landing-hero field. Size only
   * changes padding, text size, and the icon; colors/behavior are shared.
   */
  size?: 'sm' | 'lg';
  autoFocus?: boolean;
  /** Wrapper class for width/position — supplied by the parent that mounts it. */
  className?: string;
}
...
export default function HeaderSearch({ placeholder = 'Search…', size = 'sm', autoFocus, className = '' }: HeaderSearchProps) {
  ...
  // One shared look; `size` only scales geometry (padding / text / icon).
  const isLg = size === 'lg';
  const inputClass = [
    'w-full bg-surface-2 border border-border rounded-md font-mono text-foreground',
    'placeholder:text-muted focus:outline-none focus:border-[var(--color-role)] transition-colors',
    isLg ? 'pl-11 pr-4 py-3.5 text-base' : 'pl-9 pr-3 py-1.5 text-sm',
  ].join(' ');
  const iconClass = [
    'absolute top-1/2 -translate-y-1/2 text-muted pointer-events-none z-10',
    isLg ? 'left-4 w-4 h-4' : 'left-3 w-3.5 h-3.5',
  ].join(' ');
