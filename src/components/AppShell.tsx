...
export function AppShell({ children }: AppShellProps) {
  ...
  return (
    ...
    <div className="flex flex-col min-h-screen bg-background" data-role={role}>
      ...
      {/* ── Full-width top bar ─────────────────────────────────────────────── */}
      <header className="sticky top-0 z-50 h-14 px-4 bg-surface border-b border-border shrink-0">
        <div className="flex items-center h-full gap-3">
          ...
          {/* ── Search (fan role only) ───────────────────────────────────── */}
          {role === 'fan' ? (
            <>
              {/* Desktop (≥sm): fixed-width search bar always in header */}
              ...
              {/* Mobile (<sm): search icon when closed */}
              {!searchOpen && (
                <button
              ...
              {/* Spacer (desktop only): pushes NotifBell to the far right */}
              <div className="hidden sm:block flex-1" />
            </>
          ) : (
            /* Non-fan: spacer fills gap before NotifBell */
