...
export default function EditCreatorSlugPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    ...
    // Resolve slug → user ID → fetch full creator
    creatorsApi.bySlug(slug)
      .then((res) => {
        if (res.match === 'redirect') {
          router.replace(`/${res.current_slug}/edit`);
          return Promise.reject('redirect');
        }
        return creatorsApi.get(res.user.id);
      })
      .then((res) => {
        const s = res.data;
        if (!s.can_edit) {
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-10 text-center">
        <div className="bg-surface border border-border rounded-xl p-8">
          ...
          <Link href={`/${slug}`} className="text-creator hover:underline text-sm">
            ← Back to profile
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      <div className="mb-6 flex items-center gap-3">
        <Link href={`/${slug}`} className="text-muted hover:text-foreground transition-colors text-sm">
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/${slug}`} className="text-sm text-muted hover:text-foreground transition-colors">
            Cancel
          </Link>
        </div>
