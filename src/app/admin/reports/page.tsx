...
import type { BountyReportRow } from '@/lib/types';
...
const REASON_LABELS: Record<BountyReportRow['reason'], string> = {
...
const STATUS_TONE: Record<BountyReportRow['status'], 'warn' | 'good' | 'bad' | 'default'> = {
  pending:   'warn',
  reviewed:  'default',
  actioned:  'bad',
  dismissed: 'good',
    ...
    <Modal title={`Resolve report #${report.id}`} onClose={onClose}>
      <div className="space-y-3">
        <p className="text-sm text-muted">
          ...
          {' '}on{' '}
          <Link href={`/bounties/${report.bounty_id}`} className="underline underline-offset-2" target="_blank">
            {report.bounty?.title ?? `bounty #${report.bounty_id}`}
          </Link>
          {report.details && <><br />&ldquo;{report.details}&rdquo;</>}
        </p>
        <p className="text-xs text-muted/70">
          Resolving never touches the bounty itself — revoke it from the bounty page first if
          action is warranted, then mark this <em>actioned</em>.
        </p>
        <Textarea
          value={notes}
...
export default function AdminReportsPage() {
  ...
  const [reports, setReports] = useState<BountyReportRow[]>([]);
  const [statusFilter, setStatusFilter] = useState<'pending' | 'all'>('pending');
  const [loading, setLoading] = useState(true);
  const [resolving, setResolving] = useState<BountyReportRow | null>(null);
            ...
            {reports.map((r) => (
              <div key={r.id} className="py-3 flex items-start justify-between gap-4">
                <div className="min-w-0">
                  ...
                  >
                    {r.bounty?.title ?? `Bounty #${r.bounty_id}`}
                  </Link>
                  <p className="text-xs text-muted mt-0.5">
                    by {r.reporter?.display_name ?? `user #${r.id}`}
